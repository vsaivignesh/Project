...
public class LendBooks extends AppCompatActivity {
        ...
        editTextSearch.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                if(useList!=null){
                    ...
                    recyclerView.setLayoutManager(new LinearLayoutManager(LendBooks.this));
                    adapter=new aAdapter(LendBooks.this,filteredList);
            ...
            call.enqueue(new Callback<List<Avail>>() {
                ...
                public void onResponse(Call<List<Avail>> call, Response<List<Avail>> response) {
                    if(response.body()!=null){
                        Toast.makeText(LendBooks.this, "Getting Books", Toast.LENGTH_SHORT).show();
                        aData= new ArrayList<>(response.body());
                        useList=new ArrayList<>();
                        for(int i=0;i<response.body().size();i++){
                            useList.add(response.body().get(i).getBName());}
                        aAdapter aadapter=new aAdapter(LendBooks.this, aData);
                        recyclerView.setAdapter(aadapter);
                    }
                    else{
                        Toast.makeText(LendBooks.this,"No Books Available",Toast.LENGTH_LONG).show();}

                }

                @Override
                public void onFailure(Call<List<Avail>> call, Throwable t) {
                    Toast.makeText(LendBooks.this, t.getMessage(), Toast.LENGTH_SHORT).show();
